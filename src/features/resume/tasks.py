...
# --- 1. 履歷分析任務 ---
def create_analysis_task(agent, resume_data, survey_data):
    return Task(
        description=f"""
        身為 FAANG 等級的資深招募顧問，你的任務是執行極其嚴苛的「第一輪履歷篩選與風險評估」。
        請對輸入的 [使用者問卷]: {survey_data} 與 [使用者履歷]: {resume_data} 進行交叉比對。
        ...
        """,
        expected_output="一份純文字的診斷分析報告，包含【清楚度、證據力、關鍵字、一致性】分析與風險警示。",
        agent=agent
    )

# --- 2. 履歷優化任務 ---
def create_optimization_task(agent, resume_data, analysis_result):
    return Task(
        description=f"""
        你現在是一位資深履歷策略顧問。
        你的目標是將 [原始履歷內容]: {resume_data} 根據 [履歷診斷分析結果]: {analysis_result} 進行優化。
        ...
        """,
        expected_output="優化後的完整履歷內容（包含風格定義與完整履歷全文）。",
        agent=agent
    )
